...
static void key_event(uint16_t virtual_key, bool key_down, uint64_t modifiers_flags) {
     ...
     */
    CGEventSourceRef event_source = CGEventSourceCreate(kCGEventSourceStateCombinedSessionState);
    ...
    // purposefully clear the mask of the modifier flags from previous Shift.
    CGEventFlags flags = ~kCGEventFlagMaskShift & CGEventGetFlags(key_event);
     ...
     * @flags :
     */
    CGEventSetFlags(key_event, flags | modifiers_flags);
